...
public class ConvexClip
{
	 ...
	 * 
	 * @param list1
	 *            first polygon for the intersection
	 * @param list2
	 *            second polygon for the intersection
	...
	/**
	 * Using two counterclockwise oriented cVertexLists to compute the
	 * intersection of the corresponding polygons
	 * 
	 * @param p
	 *            first counterclockwise oriented polygon
	 * @param q
	 *            second counterclockwise oriented polygon
	 * @param n
	...
	{
		CVertex currp = p.head, currq = q.head; // current vertex of both
												// polygons
		InsideFlag flg = InsideFlag.UNKNOWN; // Information flag whether p or q
												// is on the inside (or if it is
												// unknown)
		int ap = 0, aq = 0; // counter for the termination condition(ap =
							// advance p,aq = advance q)
		...
		do {
			InfoPoint c = intersect(currp.prev.v, currp.v, currq.prev.v,
					currq.v); // Intersection of two polygon edges
			vQ = new Point2D(currq.v.x - currq.prev.v.x,
					currq.v.y - currq.prev.v.y);
			vP = new Point2D(currp.v.x - currp.prev.v.x,
					currp.v.y - currp.prev.v.y);
			int cross = AreaSign(nil, vP, vQ); // sign of crossproduct of vP and
												// vQ
			int pInQ = AreaSign(currq.prev.v, currq.v, currp.v); // if currp is
																	// on the
																	// half
																	// plane of
																	// q
			int qInP = AreaSign(currp.prev.v, currp.v, currq.v); // if currq is
																	// on the
																	// half
																	// plane of
																	// p
			if (c.code == '1') { // Proper intersection
				...
				inters.InsertBeforeHead(new CVertex(c.erg)); // Adding the
																// intersection
																// to the result
				// Flag update
				if (pInQ > 0) {
			...
			/*
			 * Termination condition: if ap >= n and aq >=m then both polygons
			 * traversed if ap >= 2*n or aq >= 2*m then p or q cycled twice and
			 * there will not be another intersection.
			 */
	 ...
	 * @param c
	 * @param d
	 * @return Infopoint with the flag of the intersection type and the point of
	 *         the intersection
	 * 
	 */
	public InfoPoint intersect(Point2D a, Point2D b, Point2D c, Point2D d)
	{
		...
		t = (a.x * (d.y - c.y) - a.y * (d.x - c.x) + c.y * (d.x - c.x)
				- c.x * (d.y - c.y)) / denum;
		s = ((b.x - a.x) * a.y + c.x * (b.y - a.y) - a.x * (b.y - a.y)
				- c.y * (b.x - a.x)) / -denum;
		if (t >= 0 && t <= 1 && s >= 0 && s <= 1) {
			return new InfoPoint(
					new Point2D(a.x + t * (b.x - a.x), a.y + t * (b.y - a.y)),
