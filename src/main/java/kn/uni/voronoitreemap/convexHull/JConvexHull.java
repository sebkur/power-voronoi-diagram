...
public class JConvexHull
{
	...
	/**
	 * Computation method for the convex hull, after the algorithm in the Book
	 * of Mark de Berg and the others.
	 * 
	...
	public List<JFace> compute()
	{
		...
		while (current < points.size()) {
			...
			// Iterate over horizon edges and create new faces oriented with the
			// marked face 3rd unused point
			for (HEdge hE : horizon) {
				JFace fn = new JFace(next, hE.getOrigin(), hE.getDest(),
						hE.getTwin().getNext().getDest());
	...
	/**
	 * Conflicts of the new JFace can be only the conflicts of the incident
	 * JFaces of the horizon edge
	 * 
	 * @param old1
	 *            incident facet of the horizon edge
	 * @param old2
	 *            other incident facet of the horizon edge
	 * @param fn
	 *            newly created facet, which conflicts are added
	 */
	private void addConflicts(JFace old1, JFace old2, JFace fn)
	{
		...
		while (i < l1.size() || l < l2.size()) {
			if (i < l1.size() && l < l2.size()) {
				v1 = l1.get(i);
				v2 = l2.get(l);
				// If the index is the same, its the same vertex and only 1 has
				// to be added
	...
	/**
	 * Prepares the convex hull computation Builds the tetrahedron, fills the
	 * conflict graph and builds permutation for the points list
	 */
	private void prep()
	{
		...
		for (int i = 2; i < points.size(); ++i) {
			if (!(v0.linearDependent(points.get(i))
					&& v1.linearDependent(points.get(i)))) { // not linearly
																// dependent to
																// both Vectors
				v2 = points.get(i);
				v2.setIndex(2);
		...
		for (int i = 3; i < points.size(); ++i) {
			if ((f0.getNormal().dot(f0.getVertex(0)) != f0.getNormal()
					.dot(points.get(i)))) {// Point is valid
				v3 = points.get(i);
				v3.setIndex(3);
