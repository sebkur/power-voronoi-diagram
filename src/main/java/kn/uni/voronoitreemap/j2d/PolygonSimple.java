...
public class PolygonSimple implements Shape, Cloneable, Iterable<Point2D>
{
	 ...
	 * 
	 * @param numberPoints
	 *            initial array size, default initial array size is 16.
	 */
	 ...
	 * 
	 * @param xPoints
	 *            x-coordinate of the polygon points
	 * @param yPoints
	 *            y-coordinate of the polygon points
	 * @param length
	 *            number of elements which should be considered from the given
	 *            arrays
	 */
	...
	/**
	 * Replaces the pointers of the coordinate arrays to show to the given
	 * coordinate arrays.
	 */
	...
	/**
	 * tests whether each corner point of the given rectangle is contained in
	 * the polygon.
	 */
	@Override
	...
	/**
	 * Returns the path iterator for the shape(rather slow compared to using the
	...
	/**
	 * tests whether the given rectangle will intersect to the bounds of the
	 * polygon.
	 */
	@Override
	...
	/**
	 * Adds a point to the polygon. Extends the corresponding array if
	 * necessary.
	 */
	...
	public PolygonSimple convexClip(PolygonSimple poly)
	{
		...
		if (!this.getBounds2D().intersects(poly.getBounds2D()))
			return null;
		// check if bounding box corners are in polygon: then poly is contained
...
//		list2.PrintVertices();
		...
		// no intersection between the two polygons, so check if one is inside
		// the other
	...
	{
		PolygonSimple p = new PolygonSimple(this.getXPoints(),
				this.getYPoints(), length);
	...
	/**
	 * Computes the intersection point iff (inx,iny) is within the polygon and
	 * (outx,outy) is outside of the polygon. Returns null if the is no
	 * intersection or the line crosses the complete polygon.
	 * 
	 * @param inx
	 *            x-coordinate of the presumably inner point
	 * @param iny
	 *            y-coordiante of the presumably inner point
	 * @param outx
	 *            x-coordinate of the presumably outer point
	 * @param outy
	 *            y-coordinate of the presumably outer point
	 * @return Intersection Point
	 */
	public Point2D getIntersectionWithPolygon(Double inx, Double iny,
			Double outx, Double outy)
	{
		...
		int k = 0;
		for (int i = 0, j = length - 1; i < length; j = i++) {
			denum = ((x[i] - x[j]) * (iny - outy)
					- (inx - outx) * (y[i] - y[j]));
			if (denum == 0)
				continue;
			t = (outx * (y[i] - y[j]) - outy * (x[i] - x[j])
					+ y[j] * (x[i] - x[j]) - x[j] * (y[i] - y[j])) / denum;
			s = (outy * (inx - outx) + x[j] * (iny - outy) - outx * (iny - outy)
					- y[j] * (inx - outx)) / -denum;
			if (t > 0 && t <= 1 && s > 0 && s <= 1)
				intersections[k++] = new Point2D(outx + t * (inx - outx),
						outy + t * (iny - outy));
		}
		if (k > 1)
			return null;
			...
			Point2D p4)
	{
		...
		if (Geometry.crossProduct(n1, n2)
				* Geometry.crossProduct(n3, n4) >= 0) {
			return null;
		}
	...
	/**
	 * If the polygon is modified by e.g. shrinking, this method returns the
	 * original polygon. If the polyogn was not modified, it can return null.
	 * 
	 * @return
	 */
